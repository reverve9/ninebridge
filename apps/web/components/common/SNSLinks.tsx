...
function getSNSItems(settings: SiteSettings): SNSItem[] {
  const entries: [string, string | null][] = [
    ['kakao', settings.sns_kakao],
    ['instagram', settings.sns_instagram],
    ['youtube', settings.sns_youtube],
    ['facebook', settings.sns_facebook],
  ];
  return entries
...
export default function SNSLinks({ settings, variant }: SNSLinksProps) {
  const items = getSNSItems(settings);
        ...
        {items.map(({ key, url }) => (
          ...
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              {SNS_ICONS[key as keyof typeof SNS_ICONS]}
            </svg>
            {SNS_LABELS[key]}
          </a>
        ))}
